...
function fetchDeliver(userId) {
    $.ajax({
        url: `https://naturesu.shop/deliver.php?user_id=${userId}`,
        type: 'GET',
        dataType: 'json',
...
function fetchDelivers_admin() {
    $.ajax({
        url: 'https://naturesu.shop/deliver.php',
        type: 'GET',
        dataType: 'json',
        success: function(delivery) {
            displaydelivery(delivery);
...
function updateDeliveryStatus(deliveryId, newStatus) {
    ...
    fetch(`https://naturesu.shop/deliver.php/change?delivery_id=${deliveryId}`, requestOptions)
